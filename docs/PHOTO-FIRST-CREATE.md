...
| 2 순서 | 스팟 카드 순서를 확인·드래그 | 카드에 본문 칸을 항상 열기, 주소 입력, 이동 수정 |
...
- 탭: 바텀시트 — 사진 그리드(내부 DnD·대표), 이름 수정(선택), 위치 미세조정(선택). 카드 **기본 뷰에 textarea 없음.** 「이 곳에서 남긴 말」은 시트에 여백 있게 열려 있음(접지 않음, 자동 포커스 없음, 안 써도 됨). 쓴 글이 있으면 히어로 아래 최대 두 줄.
