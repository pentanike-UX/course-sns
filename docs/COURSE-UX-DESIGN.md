...
- [x] 컬러 문서 정합: `DESIGN-SYSTEM.md` ↔ `globals.css` (브랜드 레드)
- [x] 전역 카피: 내 일기/루트 → 내 코스/코스 (사용자 노출면)
- [x] 카드·상세 위계: 따라감/다녀옴 > 좋아요, 추천대상 > 감정
- [x] 필터 순서: 지역 → 누구와 → 난이도 → 종류/테마 → 감정(접기)
- [x] FAB·작성·따라가기·완주·브랜드 마이크로카피

## Phase 1 적용 상태

- [x] 카드 실행 스펙 한 줄 (`course-spec` + legs 집계)
- [x] 정렬: 많이 따라간 / 많이 다녀온
- [x] 따라가기 후 `?followed=1` next-step 가이드

## Phase 2 적용 상태

- [x] 보관함: **따라가는 중 | 저장 | 구독 코스**
- [x] 따라가는 중 = `route_copies` + 상태 뱃지(다듬는 중/실행 준비/다녀옴) + 완주 유도 링크
- [x] 구독 코스 = **새 코스 스트림** +「팔로우 관리」demote (FOL-01)

## Phase 3 적용 상태

- [x] 상세 CTA 상태머신: 복제 후 Primary 다녀왔어요 / Secondary 내 초안 (`CourseFollowActions`)
- [x] 재복제 방지: 기존 초안 deep-link
- [x] 계획→기록 전환 카피 「기록으로 바꾸기」(완주와 분리)
- [x] 상세 위계: 요약·CTA → 스팟 → 지도 → 작성자
- [x] liked 데드링크 제거 · 댓글 에러 노출 · 삭제/나가기 착지 · 알림 링크

## Polish 적용 상태

- [x] persistence 키 `course:*` 이관 (legacy `routdiary:*` 읽기 폴백)
- [x] RouteCard 실행 스펙 한 줄
...
> **기준 코드:** course-sns MVP (`v0.3.18-mvp`, routdiary fork) · 브랜드 **coursee**  
...
| 저장/좋아요가 보관함 주연 | 따라간 코스·구독 코스가 주연 |
...
| 보관함 | `/library` | P2·P4 | **내 실행함 + 구독** — 따라간/저장한 코스, 구독 코스 스트림 |
...
| 구독 코스 | **새 코스 스트림** +「팔로우 관리」 | P4 |

- `좋아요` 보관함 탭은 제거됨(상세 보조 반응만 유지).
- 구독 코스 기본 랜딩은 **사람 목록이 아니라 새 코스 피드**.
...
### 2.7 보관함 · 구독 코스 (`LibraryTabs` / `FollowingPanel`)

**목표:** P4 구독 + P2 실행함.

**적용됨:**

- 세그먼트: `따라가는 중 | 저장 | 구독 코스`
- `따라가는 중` = 내가 copy한 코스 목록
- 구독 코스: 기본 = 팔로우한 유저의 최신 public 코스 ·「팔로우 관리」로 사람 검색/언팔
...
- 보관함 `따라가는 중` + 구독 **코스 스트림**.
- 계획 상태 뱃지·완주 유도.
- 알림 위계 조정(+ 가능 시 새 코스 알림).
- **성공 기준:** copy→completion 전환율, 구독 코스 스트림에서의 따라가기.
