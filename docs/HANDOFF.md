...
- `/routes/new`, `/routes/[id]`, `/routes/[id]/edit` — 작성·상세·수정. 기록 작성·수정은 **같은 4화면** (올리기·순서·이동·공개). 계획 수정은 지도 플래너. 완료 전 **공개/비공개 명시 선택**
...
- **버전**: **`v0.4.8-mvp`** (PATCH — 문서).
- **무엇을/왜**: 도보 레그가 지도에서 자동차 도로로 보임. 네이버 Directions 정책 + `TMAP_APP_KEY` 미설정 폴백. 도보·자전거·버스·지하철·기차로 길을 이을 수 있는 방법만 조사해 가이드에 남김. **코드 변경 없음.**
- **주요 파일**: `docs/MAP-ROUTING.md` · `/deliverables/development` · architecture · api · status · HANDOFF §6
- **다음 구현 우선**: A) TMAP 키만 (도보 차도 위장 제거) → B/C) Transit 또는 ODsay (노선). 자전거 전용은 카카오 제휴.
- **검증**: `/deliverables/development` 「스팟 이동 경로」 표. `pnpm lint`.
...
- **버전**: **`v0.4.6-mvp`** (PATCH).
- **무엇을/왜**: 등록·수정에서 스팟별 글을 받고 싶지만 4화면·히어로 카드를 복잡하게 만들지 않음. 순서 카드에는 textarea 없음. 쓴 글이 있을 때만 사진 아래 최대 두 줄. peek에서 「이 곳에서 남긴 말」을 접지 않고 여백 있게 보여 줌(자동 포커스 없음). 이동 화면 compact 카드는 그대로.
- **주요 파일**: `SpotTimelineCard.tsx` · `RouteForm.tsx` · `docs/PHOTO-FIRST-CREATE.md`
- **검증**: 기록 작성/수정 → 순서에서 카드 탭 → 메모 칸이 바로 보임. 저장 후 카드에 한 줄. 이동 화면에는 글 없음. `pnpm lint`.

### Safari 공유 버튼 위치 안내 (Cursor, 2026-08-13 · v0.4.5-mvp)

- **버전**: **`v0.4.5-mvp`** (PATCH).
- **무엇을/왜**: 아이폰 Safari 공유는 아래에만 있지 않음. 탭 레이아웃 콤팩트(•••) / 하단 / 상단에 따라 위치가 다름. 세 곳을 그림으로 안내. 아이패드 상단 도구 막대도 한 줄.
- **주요 파일**: `IosHomeScreenGuide.tsx` · `ProfileActions.tsx`
- **검증**: 설정 → 다운로드(iPhone UA). 1단계에 콤팩트·하단·상단 카드 3개.

### 아이폰 홈 화면 추가 안내 (Cursor, 2026-08-13 · v0.4.4-mvp)

- **버전**: **`v0.4.4-mvp`** (PATCH).
- **무엇을/왜**: iPhone은 PWA 자동 설치가 없어 글 두 줄만으로는 따라가기 어려움. Safari 공유 → 홈 화면에 추가 → 추가 확인을 그림 단계로 안내.
- **주요 파일**: `IosHomeScreenGuide.tsx` · `ProfileActions.tsx` · `ActionBottomSheet.tsx`(긴 시트 스크롤)
- **검증**: 설정 → 다운로드. iPhone/Safari UA면 3단계 그림 시트. Android Chrome은 기존 자동 설치.

### 설정 다운로드(PWA 설치) 버튼 수정 (Cursor, 2026-08-13 · v0.4.3-mvp)

- **버전**: **`v0.4.3-mvp`** (PATCH).
- **무엇을/왜**: 설정 헤더 다운로드를 눌러도 반응이 없음. `beforeinstallprompt`는 `preventDefault`됐지만, 설치 시트가 `open=true`로 마운트되며 enter 애니메이션이 스킵되어 화면 밖에 가려짐. 프롬프트가 있으면 탭에서 바로 `prompt()`, 시트는 항상 마운트하고 `open`만 토글.
- **주요 파일**: `use-sheet-transition.ts` · `ProfileActions.tsx`
- **검증**: 설정 → 다운로드. Chrome이면 설치 대화상자 또는 안내 시트. 공유 토스트가 드로어 위에 보임.

### 작업 완료 시 main 머지 규칙 (Cursor, 2026-08-13 · v0.4.2-mvp)

- **버전**: **`v0.4.2-mvp`** (PATCH — 에이전트 규칙).
- **무엇을/왜**: 초안 PR에 멈추지 말고, 작업이 끝나면 `main` 머지(프로덕션)까지 한다. 사용자가 머지하지 말라고 명시한 경우만 예외.
- **주요 파일**: `AGENTS.md` 「PR·프로덕션 머지」
- **검증**: 이 PR을 ready로 만들고 `main`에 머지.

### Wave G4 기록 수정 4화면 (Cursor, 2026-08-13 · v0.4.1-mvp)

- **버전**: **`v0.4.1-mvp`** (PATCH — Wave G 기록 수정을 작성과 같은 셸로).
- **무엇을/왜**: 등록 UI는 4화면인데 수정만 한 페이지 스크롤(장소·이동·이야기·공개)이라 멘탈모델이 갈라짐. 기록 수정도 올리기·순서·이동·공개. 기본은 2(순서). `?photos=1`·따라가기 초안(사진 없음)은 1(올리기). 계획 수정은 플래너 유지.
- **주요 파일**: `RouteForm.tsx` 기록 edit 분기 제거 · `edit/page.tsx` `photos` · `ConvertPlanButton.tsx` · `RouteFormLoadingShell.tsx` · peek 사진 추가/삭제
- **검증**: `pnpm lint` 해당 파일. 내 기록 수정 → 순서 카드 화면. 카드 탭 → 사진 추가. 계획→기록 전환 → 올리기 화면.
