...
### Wave B — 루프 전환율 (P1→P2, P2 완주)

1. CTA 톤 상태머신 (색+라벨)  
2. 저장 카드에「따라가기」  
3. 「따라가는 중」진행 체크리스트 1줄  
4. AuthGate 전이 가치 카피  
5. 정렬 UI에서 popular 퇴장  

**성공 기준:** Detail→Copy, Copy→Completion 가드레일 지표 개선.

### Wave C — 브랜드 토큰 건강성

1. success를 brand에서 분리한 토큰 확정 → `globals.css` + DESIGN-SYSTEM 갱신  
2. 플래너 레드 예산(primary 1개)  
3. 지도 walk 색 분리  
4. `--primary-green-*` / `leaf` deprecate 주석·린트 가이드  

**성공 기준:** 신규 PR에서 상태색을 brand로 오용하는 회귀 0.

### Wave D — 영향력·구독 (P3·P4)

1. 프로필/책장 지표 위계를 전이 중심으로  
2. 팔로잉 스트림·알림 위계  
3. 게시 전 따라가기 준비도 힌트  
