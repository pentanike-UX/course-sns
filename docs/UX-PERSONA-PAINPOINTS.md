...
> **범위:** course-sns MVP `v0.2.0-mvp` (코드 + DESIGN-SYSTEM · COURSE-UX)  
> **목적:** Waves A–D·공개 게이트 반영 후 P1–P4를 **10점 시나리오**로 다시 돌리고, 잔여 페인포인트와 UX 개선안(Wave E) 도출  
> **기준일:** 2026-07-23  
> **이전 분석:** 동 문서 Wave 이전판(2026-07-22) — §5 Wave A–D는 ✅ 적용됨

---

## 0. 한 줄 요약

전이 언어·CTA 톤·토큰 분리·영향력 지표는 **중·후반 퍼널(P2 실행 · P3 게시 안전)을 크게 올렸다.**  
남은 병목은 **콜드 발견 프루프**, **상세에서 좋아요가 전이 CTA와 경쟁**, **P4 구독 배달(새 코스 알림/홈 레일)**, **진행 체크리스트 진정성**이다.
...
| X2 | 상세 좋아요가 북스타와 경쟁 | 🔴 잔여 — Wave E1 |
| X3 | error ≈ brand 레드 | 🔴 잔여 — Wave E5 |
| X4 | 버전·작업로그 미갱신 | ✅ `v0.2.0-mvp` + HANDOFF §7 |
...
1. `RouteView` 소셜 블록: **따라가기/다녀왔어요를 ♥·저장보다 위**(또는 ♥를 overflow/접기)
2. like/bookmark AuthGate에도 한 줄 전이 맥락(또는 보조 액션임을 명시)
3. 성공 기준: 상세 첫 스크롤 없이 주 CTA = 전이

### E2 — 콜드 발견·지도 패리티 (P1, 침습 낮음)

1. Transfer 슬롯 고정: 0이어도 `첫 따라가기` / `아직 후기 없음` 카피
2. TourCard에 `courseSpecParts` 패리티
3. SpecLine: 최소 `지역 · 스팟 N` 폴백으로 null 금지
4. 성공 기준: 홈·지도 1초 테스트 동일

### E3 — P4 구독 배달 (P4·P3, 침습 중) 🎯🎯

1. 알림 타입 `course_publish`(또는 동등) — 팔로잉 메이커 공개 시
2. (병행) 홈 상단 약한 레일「팔로잉의 새 코스」— 데이터 있을 때만
3. 메이커 측: 따라가기 수신 알림/카운트 토스트(P3-F)
4. 성공 기준: Follow→Copy가 보관함 외 경로로도 성립

### E4 — 진행 체크리스트 진정성 (P2, 침습 낮음~중)

1. `스팟 확인`: 스팟≥1 + (제목 또는 메모) 등 실데이터 조건
2. `이동 확인`: legs 수단/시간 중 1개 이상
3. 저장 탭 vs 따라가는 중: 뱃지/아이콘 차별 강화(오버레이 카피 `저장함 · 아직 안 따라감`)
4. 성공 기준: 체크리스트가 “장식”이 아니라 next-step 진실원

### E5 — 신뢰 토큰 마감 (P3·공통, 침습 낮음)

1. `--error`를 brand와 채도/색상 분리(예: 더 어두운 rose/crimson 또는 문서화된 destructive)
2. 통계 타이틀 `여행 통계` → `코스 통계` / `내 영향력`
3. 성공 기준: 에러 wash ≠ CTA wash

### E6 — 팔로잉 IA 단순화 (P4, 침습 중, E3 이후)

1. 기본=스트림 유지, 사람 관리를 검색 시트/톱니로 강등해 서브칩 제거 검토
2. FollowToggle AuthGate: “새 코스를 보관함에서 받아보려면 로그인”
