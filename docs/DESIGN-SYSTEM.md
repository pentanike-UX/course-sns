...
> 최종 업데이트: 2026-08-03 · course-sns MVP `v0.3.10-mvp` · **브랜드 = coursee** (공식 SVG 라이트/다크 락업 · C+모션 도트) · 포인트 레드 · 표면/잉크 뉴트럴 · success=teal · **error≠brand**
...
디자인 변경
  ...
  3. src/lib/version.ts     (APP_VERSION — 사소한 수정도 PATCH+)
  4. docs/HANDOFF.md §7     (버전·맥락·검증)
  5. deliverables/changelog · README
  6. 컴포넌트 JSDoc          (비표준 동작만)
```

Figma 파일 없음 — **스크린샷 + HANDOFF 작업 로그**가 변경 이력.  
에이전트 필수 규칙: [`AGENTS.md`](../AGENTS.md) 「버전·작업 기록」(예외 없음).
